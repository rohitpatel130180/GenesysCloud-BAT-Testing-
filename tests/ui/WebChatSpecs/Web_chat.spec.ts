...
test(' User navigating through Bereavement intent flow ', async ({ page, webChatUtils, webChatActions }) => {
  ...
  expect(await webChatUtils.getChatbotRoboResponse()).toBe("Please tell me what type of query you have so I can help you find the right information, e.g. billing query, my online account, meter readings");
  await webChatUtils.sendMessage('My partner died');

  annotate("Then User receives bereavement guidance from the chatbot.");
  expect(await webChatUtils.getChatbotRoboResponse()).toBe("If you need to let us know that one of our customers has passed away, we’re here to help you through the next steps.");
  expect(await webChatUtils.getChatbotRoboResponse()).toBe("Have you already contacted us to advise someone has passed away?");
  ...
  expect(await webChatUtils.getChatbotRoboResponse()).toBe("Do you still need help and would like to talk to one of our agents?");
  await webChatUtils.userClickNoButton();

  annotate("And Chatbot asks if there is anything else; user selects 'No' again.");
  expect(await webChatUtils.getChatbotRoboResponse()).toBe("Is there anything else I can help with?");
  await webChatUtils.userClickNoButton();

  annotate("Then Chatbot ends the conversation with a thank you message.");
  expect(await webChatUtils.getChatbotRoboResponse()).toBe("Thank you for contacting OVO Energy. If you have any further questions just ask me anytime...");
